...
- `handoff.py` builds a new folder's `meta.yaml` from a scaffold dict, so a NEW application will
  ...
  **Decided (2026-07-30):** the scaffold change is **not** in 7b. `handoff.py` is public code that
  ...
  [`2026-07-31-handoff-scaffold-omits-company-key`](../../4_done/2026-07-31-handoff-scaffold-omits-company-key/task.md)
  (link re-pointed 2026-08-02: that task was picked up and is now in `4_done`; the decision
  above stands — the scaffold still resolves nothing, it just says so).
