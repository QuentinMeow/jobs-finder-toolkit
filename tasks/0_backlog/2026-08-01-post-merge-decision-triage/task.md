...
| 2 | `first-search-recency-window.md` | Should a company's FIRST-ever automated search use a wider recency window? | Narrow recency window on every run, including a company's first-ever search — real, still-open, fully-matching postings get dropped with no trace (2 confirmed instances). | Recommended Option B auto-widens the window only on a first search, printed in the run header, narrowing back afterward. | 5 |
...
| 5 | `handoff-records-non-clean-scaffolds.md` | Should handoff record a posting whose scaffold came out incomplete? | Already implemented as Option A: every created folder is skip-logged regardless of exit code, so an abandoned/incomplete scaffold is permanently skipped unless you run the printed `--forget-log` command. | Mostly a confirmation — Option B would need a real code change to stop recording non-clean scaffolds, reopening the bug this fixed. | 3 |
...
| 14 | `subagent-budget-cap-conflicts-with-long-sessions.md` | The 8-subagent cap conflicts with sessions you actually ask for — scope it, exempt it, or enforce it? | Cap is unenforced and unenforceable today; the owner already routinely and correctly overrides it for long autonomous sessions. | Recommended Option A scopes the cap to search/application fan-out only, and requires any wider session to report its total — makes the written rule match observed practice. | 5 |
