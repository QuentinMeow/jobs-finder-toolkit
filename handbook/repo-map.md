...
| `automation/shared/job_metadata.py` | **Canonical** pure extractor/validator for the flat schema-v4 per-posting `status`, job level (normalized word + approximate Google-equivalent range), required YOE, salary, `workplace` (onsite/hybrid/remote), and heuristic `sponsorship` (likely/unlikely/unknown), plus the `derive_status` folder rollup and loading of the optional sourced company-levels reference cache. Vendored into all three job workflow skills |
| `automation/shared/metadata_editor.py` | **Canonical** formatting-preserving schema-v4 `meta.yaml` editor (YAML node anchors, checksums, atomic writes, semantic verification, idempotence). Vendored into application-tracker |
...
| `skills/outlook-email-assistant/` | PUBLIC draft-only personal Outlook workflow: reads mailbox messages, grounds suggested replies in the private job-hunt data, and creates Outlook drafts through Microsoft Graph. OAuth tokens stay in the OS keyring; the skill has no send capability. |
