...
`gardener`. **PRIVATE skill**: `coding-interview` — the entire skill lives only in the overlay.
...
2. Read the relevant PUBLIC skill before working. Skills are **quickstart-first**: the SKILL.md
   ...
   `behavioral-interview-prep`, `company-research` (company/role research + question bank). Private
   `coding-interview` is at `.agents/skills/coding-interview/` when the overlay is mounted.
