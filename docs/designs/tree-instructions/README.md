...
[docs/handbook/doc-style.md](../../handbook/doc-style.md).
...
AGENTS.md (root) — invariants + repo map + router lines, e.g.:
    │   "Working under docs/designs/? Read docs/designs/AGENTS.md first
    │    (skip if your tool already injected it)."
    ▼   loaded lazily — only when an agent works in that folder
docs/designs/AGENTS.md — THE one seed leaf: 8 lines of pure pointers
...
docs/handbook/doc-style.md / future agents-references/ files
```
...
(`docs/designs/`); the message-queue/ leaf from v1 was **deleted** after review
...
| References | `<folder>/agents-references/*.md` (and `docs/handbook/doc-style.md`, grandfathered as this tier for its folder) | Only via a task-conditioned pointer                                                                      | ≤300 lines per file                      | Unbounded detail                                                            |
...
tree: **one** leaf — `docs/designs/` (pure pointers to its binding style
...
| The `docs/designs/` leaf inlined compressed copies of the style contract instead of pointing at it — the exact drift vector the design warns about. (major)                                                                                   | Stripped to 8 lines of pure pointers; the relocation-or-pointer rule added — [The three tiers](#1-the-three-tiers).                                |
