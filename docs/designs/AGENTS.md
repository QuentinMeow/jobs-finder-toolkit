# docs/designs/ — local pointers (additive to the root contract)

- Before writing, editing, or restructuring ANY design doc here, read
  [`../handbook/doc-style.md`](../handbook/doc-style.md) — the binding writing contract for design docs.
- Each subfolder is one design family; its `README.md` is the family index.
  Historical families are records — never rewrite their conclusions.
- Root `AGENTS.md` rules apply unchanged (leak guard: fictional
  Jordan-Rivers examples only; async two-way doc fields).
