...
| `config.applications_jsonl_path()` | (c) durable memory | permanent, append-only; **not regenerable** — nothing rewrites it, so a deleted application keeps its row and recovery is overlay git history, never a rebuild. `handoff.py` records every folder it creates, whatever exit code the run returned, because only the owner deletes an application folder — so a deleted one means the posting is not to be reconsidered (`memory/decisions/handoff-records-every-folder-it-creates.md`). Repair a wrong row by appending a tombstone (`status.py --forget-log`), never by hand-editing | `status.py` / `handoff.py` |
