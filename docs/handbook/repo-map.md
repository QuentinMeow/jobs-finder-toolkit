...
| `config.applications_jsonl_path()` | Append-only event log of postings already generated/considered — job-search skips them (new roles at the same company still surface). `status.py --sync-log` and every `--update` append to it; nothing rewrites it, so deleting an application does not un-skip its posting |
