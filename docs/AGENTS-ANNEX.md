# AGENTS.md — Annex (extended reference)

This is the on-demand reference companion to the root `AGENTS.md` (the agent
contract). `AGENTS.md` carries the boot-critical core every agent reads before
acting; this annex holds the full detail — the complete command cookbook, the
full directory table, long rationale, edge-case policies, and setup/bootstrap
steps. Each `AGENTS.md` section points here with a "details: `docs/AGENTS-ANNEX.md`
§N" pointer. Read the relevant § when the core sends you here; you do not need to
read this file top to bottom.

## §1 Configuration (full)
...
## §3 Folder Structure (full directory table)
...
| Path | Purpose |
|------|---------|
| `config.yaml` (git-ignored) / `config.example.yaml` (tracked) | Candidate identity, paths, and output-stem config; the tracked example is the neutral "Jordan Rivers" placeholder + fallback (see §1 Configuration) |
...
| `docs/AGENTS-ANNEX.md` | This file — the agent-contract extended reference |

## §4 Handy Commands (full cookbook)
...
## §7 Sharing Code Across Skills (Vendoring)
...
## §11 Application Folder Convention (full)
...
## §12 Guardrails (extended guidance)
