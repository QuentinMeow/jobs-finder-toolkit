"""Safely preview or insert schema-v6 job metadata into application meta.yaml files.
...
def _read_exact_jd(app_dir: Path, record: dict) -> str:
    ...
    if not named:
        raise ValueError(
            ...
            "schema-v6 metadata requires an exact JD association"
        )
...
def generated_metadata_by_path(app_dir: Path, meta: dict) -> dict[tuple, dict]:
    ...
    if not isinstance(jobs, list) or not jobs:
        raise ValueError(
            "schema-v6 metadata requires a non-empty jobs list (one entry per posting)"
        )
    generated = {}
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Comma-separated status labels (default: all status folders — the "
             "fleet is uniformly schema v6). Options: "
             f"{', '.join(STATUS_DIRS)}.",
    )
