...
`jd.md`. `skills/resume-writer/scripts/check.py` reads and concatenates every `JD-*.md` in `source/`, so
...
`.venv/bin/python skills/application-tracker/scripts/status.py --check-locations`; every drafted app must
...
rewrite, and the SKILL.md Step 5 hard constraints always hold. `skills/resume-writer/scripts/check.py` enforces most of them
...
For each role, `render.py` / `skills/resume-writer/scripts/cover_letter.py` render this section into
`source/..._Cover_Letter_<job title>.docx` + `..._Cover_Letter_<job title>.pdf` (at root).
Render all of a folder's cover letters with
`.venv/bin/python skills/resume-writer/scripts/cover_letter.py <applications_root>/6_drafted/<slug>/`, or just one role's
with `.venv/bin/python skills/resume-writer/scripts/cover_letter.py <applications_root>/6_drafted/<slug>/ --label "<Role>"`.
...
**Mandatory structure (enforced by `skills/resume-writer/scripts/check.py`, `check_cover_letter`). This is not
...
1. **JD files** — save one `source/JD-<job title>.md` per posting (full JD text + URL).
   `skills/resume-writer/scripts/check.py` reads and concatenates every
...
```bash
.venv/bin/python skills/resume-writer/scripts/render.py <applications_root>/6_drafted/<company>-<roleA>-<date>/
.venv/bin/python skills/resume-writer/scripts/render.py <applications_root>/6_drafted/<company>-<roleB>-<date>/
```
