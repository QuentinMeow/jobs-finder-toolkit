...
- **max_age_days** — only postings published within the last N days (`null` = don't
  filter on posting age, which is also what the pipeline uses when the key is absent).
  `_TEMPLATE.yaml` ships an explicit `3`, so set it to `null` in your copy unless you
  want a 3-day recency window.
