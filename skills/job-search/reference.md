...
| Offer whose sentence negates, but out of the scope's reach | `We are unable, given current headcount constraints and the timeline for this particular opening, to offer visa sponsorship.` — a bound (the clause break, or the ~8-token budget) refused the cue, so the sentence said something the classifier could not read. That is not silence, and it is not a denial either: the offer is demoted, never asserted (`sponsorship.unreachable_negation.*`). Two guards keep it off real offers — the phrase must NOT head its own clause (`no relocation budget, and visa sponsorship is available` stands), and no HARD break may sit between cue and phrase (`we cannot guarantee an outcome, **but** we do provide visa sponsorship` stands). | `unclear` (review) |
